...
let gameEnded = board =>
  List.for_all(field => field == Marked(Circle) || field == Marked(Cross), board)
  ...
  oldBoard == updatedBoard
    ...
    : {
        ...
        let rec check = (rest: winningRows) => {
          ...
          switch (getWinner(flattenBoard, head), gameEnded(flattenBoard), tail) {
...
let updateBoard = (board: board, gameState: gameState, id) =>
  board |> List.mapi((ind: int, row: row) =>
    row |> List.mapi((index: int, value: field) =>
...
let initialState = {
  board: list{list{Empty, Empty, Empty}, list{Empty, Empty, Empty}, list{Empty, Empty, Empty}},
...
let make = () => {
  ...
  <div className="game">
    <Board state onRestart={_evt => dispatch(Restart)} onMark={id => dispatch(ClickSquare(id))} />
  </div>
}
